...
def construct_command(input, validator, sol_dirr):
    ...
    sol_format = [(x, i) for i, x in enumerate(order)]
    sol= [f"{sol_dirr}/{i[0]}-{i[1]}.json" for i in sol_format]
    ...
    validate_comm = f"java -jar {validator} --sce {sce_file} --his {his_file} --weeks {' '.join(week_files)} --sols {' '.join(sol)} --verbose"
    print(validate_comm)
